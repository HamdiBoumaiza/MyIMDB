...
class UpcomingFilmSource(private val api: RemoteApi) :
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Movie> {
        return try {
            ...
            val upcomingFilms = api.getUpcomingMovies(page = nextPage)
            LoadResult.Page(
                data = upcomingFilms.results.map { it.mapToMovie() },
                prevKey = if (nextPage == 1) null else nextPage - 1,
                nextKey = if (upcomingFilms.results.isEmpty()) null else upcomingFilms.page + 1
            )
        } catch (e: IOException) {
